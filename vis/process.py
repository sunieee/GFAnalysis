...
'''
papers = pd.read_csv('../out/papers/' + authorID + '.csv')
...
links = pd.read_csv('../out/links/' + authorID + '.csv')
...
for paper in papers.to_records():
    ...
    data['nodes'].append({
        ...
        'year': paper['year'],
        'citationCount': relu(paper['totalCitationCount']),
        'citeStartYear': relu(paper['citeStartYear']),
        'citationCountByYear': cumulativeCitationsByYear,
        'radius': 5 + math.sqrt(math.sqrt(relu(paper['totalCitationCount']))),
